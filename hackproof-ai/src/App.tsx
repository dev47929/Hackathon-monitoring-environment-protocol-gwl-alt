...
export default function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setShowNav(current < lastScrollY.current || current < 20);
  ...
  return (
    <div className="min-h-screen text-slate-100 flex flex-col font-sans antialiased selection:bg-emerald-500/30 selection:text-emerald-300">
      ...
      <header className={`${location.pathname === '/' ? 'fixed top-0 left-0 right-0 z-50 px-0 py-0 transition-transform duration-300 ' + (showNav ? 'translate-y-0' : '-translate-y-full') : 'relative z-50 px-0 py-0'}`}>
        <div className="relative">
          <DotField
            ...
            glowColor="#120F17"
          />
          <div className="relative z-10 px-6 py-3 md:px-10 max-w-7xl mx-auto flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
          <div className="flex items-center justify-between md:justify-start gap-4">
            <button onClick={() => navigate('/')} className="flex items-center gap-2.5 text-left group cursor-pointer">
              <div className="w-10 h-10 rounded-lg overflow-hidden shadow-lg group-hover:scale-105 transition-transform duration-200">
            ...
            <div className="flex items-center justify-end">
              {user === null ? (
                <button onClick={() => navigate('/login')}
                  className="w-full lg:w-auto flex items-center justify-center gap-1 px-4 py-2 rounded-lg text-sm font-mono font-bold bg-indigo-600 hover:bg-indigo-500 text-white transition-all cursor-pointer shadow-md shadow-indigo-600/15 border border-indigo-500/20">
                  <Lock className="w-4 h-4" /> <span>Sign In</span>
                </button>
              ) : (
                <div className="w-full lg:w-auto flex items-center justify-between lg:justify-start gap-2 p-1 pl-2.5 pr-1.5 rounded-lg">
                  <div className="flex flex-col text-left">
                    <span className="text-sm font-bold text-white leading-none max-w-[120px] truncate">{user.name}</span>
                    <span className="text-xs font-mono uppercase text-indigo-400 font-extrabold tracking-wider mt-0.5">
                      {user.role === 'team' ? 'Hacker' : user.role}
                    </span>
                  </div>
                  <button type="button" onClick={authLogout}
                    className="p-1 px-2 text-slate-500 hover:text-slate-300 font-mono text-xs cursor-pointer hover:bg-slate-950 rounded border border-transparent hover:border-slate-800 transition-colors"
